...
class MarketSim:
    ...
    def simulate_event_xgb_prob(self, df_r, num_steps) -> pd.DataFrame:
        ...
        for lag in range(1, 4):  # Same number of lags as used during training
            df_r[f'Time_lag{lag}'] = df_r['Time'].shift(lag)
            df_r[f'Size_lag{lag}'] = df_r['Size'].shift(lag)
            df_r[f'Price_lag{lag}'] = df_r['Price'].shift(lag)
            df_r[f'mid_price_lag{lag}'] = df_r['mid_price'].shift(lag)
            df_r[f'calc_direction_lag{lag}'] = df_r['calc_direction'].shift(
                lag)
    ...
    def run_market_simulation(self, start_price=585.0, num_steps=34200):
        ...
        df_r = self.merge_df[['Time', 'Size', 'Price',
                              'mid_price', 'calc_direction', 'event_type']]
