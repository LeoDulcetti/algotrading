...
class MarketSim:
    ...
    def __init__(self):
        ...
        self.message_data_path = os.path.join(
            cur_dir, "..", "data", "message_data.csv")
        self.orderbook_data_path = os.path.join(
            cur_dir, "..", "data", "orderbook_data.csv")
            ...
            for col in probabilities_df.columns:
                if row[col] > 0.5:  # Check if the probability is greater than 0.1
                    ...
                    if order_type == 1:
                        ...
                        results.append(
                            {'Time': index, 'OrderType': 1, 'Direction': -1, 'Price': price, 'Volume': volume})
                        start_price = price
                        ...
                        results.append(
                            {'Time': index, 'OrderType': 4, 'Direction': -1, 'Price': price, 'Volume': volume})
                        start_price = price
                    ...
                    ############################## Partial and Full Cancelation ##############################
                        ...
                        if not results:  # Check if the list is empty
                            ...
                            results.append(
                                {'Time': index, 'OrderType': 2, 'Direction': -1, 'Price': last_order_price, 'Volume': volume})
                        ...
                        if not results:
                            ...
                            results.append(
                                {'Time': index, 'OrderType': 3, 'Direction': -1, 'Price': last_order_price, 'Volume': volume})
...
class SimulationRunner:
    ...
    def run_simulations(self, start_price=585.0, concat=True):
        ...
        for i in range(self.num_simulations):
            ...
            header = ['Time', 'OrderType', 'Direction', 'Price', 'Volume']
        ...
        print("""
            ...
            OrderType   |EventType    |Type                   |Direction
            -------------------------------------------------------------------
            1           |1;2          |Limit Order            |Sell(-1); Buy(1)
            2           |3;4          |Partial Cancelation    |Sell(-1); Buy(1)
            3           |5;6          |Full Cancelation       |Sell(-1); Buy(1)
            4           |7;8          |Market Order           |Sell(-1); Buy(1)
            -------------------------------------------------------------------
            """)
